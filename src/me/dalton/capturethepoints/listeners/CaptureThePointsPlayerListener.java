...
public class CaptureThePointsPlayerListener implements Listener {
    	...
    	if(a.getConfigOptions().eggsAreGrenades) {
        	...
        	if(pd.inLobby()) {//Player is in the lobby so don't do anything.
        		...
        		if (ctp.getArenaUtil().isInsideAB(event.getEgg().getLocation().toVector(), a.getFirstCorner(), a.getSecondCorner())
        			...
        			l.getWorld().createExplosion(l.getX(), l.getY(), l.getZ(), (float)a.getConfigOptions().grenadePower, false, false);
            		return;
        		}else {
	...
	public void shop(Arena arena, Player p, Sign sign) {
        ...
        if (ctp.getMoneyUtil().canPay(p.getName(), price)) {
            ...
            CTPShopPurchaseEvent event = new CTPShopPurchaseEvent(arena, p, playerdata, price, sign, stack);
            ...
            price = event.getCost(); //Get the cost from the event, in case someone set it to a different amount
